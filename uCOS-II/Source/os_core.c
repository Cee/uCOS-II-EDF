                   ...
                   INT16U   opt)
{
    ...
    if (ptcb != (OS_TCB *)0) {
		...
		///*
		// pext的格式如下：
		// {depedencyCount,childCount,depedencyOSTCBID[0],...,childOSTCPID[0],...}
		// */
		//if (pext != NULL)
		//{
		//	INT16U *extArray = (INT16U*)pext;
		//	ptcb->depedencyCount = extArray[0];
		//	ptcb->childCount = extArray[1];
		//	int index;
		//	int count;
		//	for (index = 2, count = 0; count < ptcb->depedencyCount; index++, count++)
		//		(ptcb->dependencyOSTCBPriority)[count] = extArray[index];
		//	for (count = 0; count < ptcb->childCount; index++, count++)
		//		(ptcb->childOSTCBPriority)[count] = extArray[index];
		//}
